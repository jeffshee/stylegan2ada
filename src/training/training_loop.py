...
def training_loop(
    ...
    if rank == 0:
        ...
        # save_image_grid(images, os.path.join(run_dir, 'fakes_init.'+ext), drange=[-1,1], grid_size=grid_size)
    ...
    while True:
        ...
        fields += [
            f"cpumem {training_stats.report0('Resources/cpu_mem_gb', psutil.Process(os.getpid()).memory_info().rss / 2 ** 30):<4.1f}"]
        fields += [
            f"gpumem {training_stats.report0('Resources/peak_gpu_mem_gb', torch.cuda.max_memory_allocated(device) / 2 ** 30):<4.1f}"]
        torch.cuda.reset_peak_memory_stats()
        fields += [
            f"aug {training_stats.report0('Progress/augment', float(augment_pipe.p.cpu()) if augment_pipe is not None else 0):.3f}"]
        # !!!
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe)]:
                ...
                if name == 'G_ema': compact_data[name] = module  # !!!
